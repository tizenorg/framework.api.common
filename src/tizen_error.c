...
static err_info err_list[] =
{
	...
	ERR_ENTRY("MEDIACODEC_ERROR_INVALID_OUTBUFFER", TIZEN_ERROR_MEDIACODEC | 0x03 ,"Invalid state"),
	...
	ERR_ENTRY("MEDIACODEC_ERROR_CODEC_NOT_FOUND", TIZEN_ERROR_MEDIACODEC | 0x07 ,"Not supported format"),
...
char* get_error_message(int err_code)
{
	...
	else if(err_code >= TIZEN_ERROR_TTS && err_code < -0x009F0000)
	{
		tmp = _get_error_message(err_code);
		if(tmp != NULL)
		{
